...
本控制器采用混合输入模式，下图展示了实际运行时的 PC 端与平板端界面。

<!-- <div align="center">
![PC端游戏与摄像头界面](assets/1.png)
  <img src="path/to/your/image_4.png" width="48%" alt="平板触控界面" />
  <p><i>左图：PC 游戏画面与摄像头 Air 判定视窗； 右图：平板触控滑条界面</i></p>
</div> -->
### 如何摆放我的摄像头和iPad
<img src="assets/1.png"  alt="摆放我的摄像头和iPad" />


如上图所示，系统通过网络连接平板与 PC，协同完成游戏输入：

* **地面触控 (Ground/Slide)**：
    在平板端（下图），屏幕被划分为 16 个垂直的触控区域，对应街机的触摸条。直接在平板上进行点击、按住或滑动，即可触发游戏中的 Tap、Hold 和 Slide 音符。

<img src="assets/3.png"  alt="iPad界面" />

* **空中判定 (Air/IR)**：
    摄像头的实时画面显示在 PC 端（下图右上角）。画面中绘制的彩色水平线代表了不同的 **AIR 判定层级 (IR1~IR6)**。
    * 当您根据谱面要求抬手时，摄像头会捕捉手部位置（画面中的绿色光点）。
    * 系统根据光点所处的高度区域，自动触发对应的空中音符。

<img src="assets/2.png"  alt="电脑界面" />

通过这种方式，您可以像在街机上一样，流畅地交替使用双手完成地面与空中的混合操作。
